...
-- Usage:
/*
-- Search ALL LinkedServers for matching expression:
 USE [DBA]
 GO
  
 DECLARE @ContainsSearchCondition varchar(255) = 'course'; -- Searches on either TableName, ColumnName, Comments or ColumnComments.
 SELECT * from udf_GetTableColumnCommentsResults(@ContainsSearchCondition, DEFAULT, DEFAULT, DEFAULT, DEFAULT, 1)
 ...
 DECLARE @ContainsSearchCondition varchar(255) = 'course'; -- Searches on either TableName, ColumnName, Comments or ColumnComments.
 SELECT * from udf_GetTableColumnCommentsResults(@ContainsSearchCondition, 'ISODS_PROD', DEFAULT, DEFAULT, DEFAULT, 1)
 ...
 DECLARE @ContainsSearchCondition varchar(255) = 'course'; -- Searches on either TableName, ColumnName, Comments or ColumnComments.
 SELECT * from udf_GetTableColumnCommentsResults(@ContainsSearchCondition, 'ISODS_PROD,SIS', DEFAULT, DEFAULT, DEFAULT, 1)
*/
-- 
-- results:
...
--	2014-04-02 by kjt: Revised logic to only include results for linked servers provided if linked servers were provided.
--						Also revised usage examples to correctly reflect updated API.
-- =============================================
...
(	
	...
	@IncludeBannerItems bit = 0 --Set to 0 to exclude Banner info.
...
BEGIN
	...
	------------------------------------------------------------------------------------------------
	--The logic is much simpler if we just delete any rows not conforming to the parameters provided:

	-- Delete any results with linked server names other than those provided:
	IF EXISTS (SELECT 1 FROM @StringValueTable WHERE String IS NOT NULL AND LEN(String) > 0) 
		DELETE FROM @returntable WHERE LinkedServerName NOT IN (SELECT * FROM @StringValueTable)
	
	-- Delete any Banner results if we were supposed to exclude them:
	IF @IncludeBannerItems = 0
		DELETE FROM @returntable WHERE LinkedServerName LIKE 'SIS%'
